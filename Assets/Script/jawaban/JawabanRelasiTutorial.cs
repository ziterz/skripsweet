...
public class JawabanRelasiTutorial : MonoBehaviour
{
    public GameObject benar, salah, lanjutkan, cek;
    public GameObject item1, item2, item3, item4, item5, target1, target2;
    public AudioSource correct, incorrect;

    public void OnCek()
	{ 
       if(item1.transform.position == target1.transform.position && item2.transform.position == target2.transform.position )
        {
            /*correct.Play(0);*/
            actionBenar();
            cek.SetActive(false);
